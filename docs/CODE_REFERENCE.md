...
- `ProjectsPage`：项目列表 + 删除
- `NewProjectPage`：创建项目
- `OutlinePage`：编辑大纲 + 触发场景拆解
- `ScenesPage`：场景树编辑 + AI 生成 + 排序
- `ChaptersPage`：章节创建、正文编辑、运行工作流
...
- `ExecutionsPage` / `ExecutionDetailPage`：执行历史与实时控制
- `SettingsPage` / `RagSyncPage`：项目标签模板与 RAG 入口页
...
- `commands/projects.rs`：本地项目扫描/创建/删除（无需 sidecar）
